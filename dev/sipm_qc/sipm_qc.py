...
class MyFrame(wx.Frame):
    def __init__(self, parent, id, title):
       ...
       ## Add display boxes
        ...
        gsbutton.AddMany([
                        ...
                        (wx.Button(self, 20, 'L = 6 dB'), 0, wx.EXPAND),
                        (wx.Button(self, 21, 'L = 9.5 dB'), 0, wx.EXPAND),
                        (wx.Button(self, 22, 'L = 12 dB'), 0, wx.EXPAND),
	...
	self.display1.Clear()
        ...
        in_file = open("./json/test.json","r")
        new_dict = json.load(in_file)
        in_file.close()
        self.display1.AppendText(str(new_dict['HV']))
    ...
    def callplotter(self, led_no):
        ...
        while True:
            infile = open("./data/sipm_%d/sipm_%d_%d.txt" % (sipm_no, sipm_no, led_no), 'r')
            for line in infile:
                amount = float(line)
                total += amount
                length = length + 1
            infile.close()
            if length == 500:
                print "LED #%d reached %d" % (led_no, length)
    ...
    def ReadTemp(self, event):
        ...
        new_dict = json.load(in_file)
        in_file.close()
        self.display3.AppendText(str("%.2f" % new_dict['Temperature']))
    ...
    def ReadGain(self, event):
        ...
        new_dict = json.load(in_file)
        in_file.close()
        self.display2.AppendText(str(new_dict['Gain']))
